...
- DAG 依赖分析（循环检测、拓扑排序）
- 并行任务调度支持
- 三种测试模式（auto/interactive/manual）
- **QA Agent** 代码审查和质量检查
- **智能错误恢复** 自动诊断和修复尝试
...
| `/harness-test` | **新增** 自动端到端测试验证 | 有待验证任务 |
| `/harness-review` | **新增** QA Agent 代码审查 | 有已完成任务 |
| `/harness-recovery` | **新增** 智能错误恢复诊断 | 测试失败后 |
...
| 模式 | 描述 | 适用场景 |
|------|------|----------|
| **auto** | Agent 自动执行测试并返回结果 | 标准功能验证（默认） |
| **interactive** | Agent 执行测试后等待用户确认 | 高风险功能 |
| **manual** | 仅提供测试指导，用户手动执行 | 特殊环境依赖 |

**配置方式**（在 `.harness/config.json`）：
...
任务开发完成 →
├── 自动调用测试 Agent
├── 使用 Playwright MCP 执行验证步骤
├── 截图记录保存到 .harness/test-results/<task_id>/
├── 返回结构化结果
└── 自动更新 passes 字段
```
...
**新增 Python 分析脚本**：`harness-core/lib/dag.py`

**可用命令**：

```bash
# 获取下一个可执行任务（考虑依赖）
python3 dag.py feature_list.json next

# 获取可并行执行的任务列表
python3 dag.py feature_list.json parallel 3

# 生成依赖图可视化（ASCII）
python3 dag.py feature_list.json graph

# 检测循环依赖
python3 dag.py feature_list.json validate

# 输出拓扑排序后的任务顺序
python3 dag.py feature_list.json order
```
...
| 触发时机 | 审查维度 | 结果处理 |
|----------|----------|----------|
| passes=true 后 | Correctness/Security/Maintainability/Performance/Testing | approve/warn/request_changes |

**审查流程**：
```
任务完成 →
├── 调用 QA Agent 分析 git diff
├── 多维度审查（Correctness 最高优先）
├── 生成审查报告 → .harness/design/<task-id>-review.md
│
├── 结果处理：
│   ├── approve → 任务确认完成，继续循环
│   ├── warn → 提示警告但继续
│   └── request_changes → 回退 passes=false，暂停修复
```
...
```json
{
  ...
  "recovery_auto_fix": true,
  "recovery_max_retries": 2,
  "recovery_record_patterns": true
}
```
